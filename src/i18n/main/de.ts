export const i18n = {
  ...
  allUnusableTip:
    'Alle Karten sind unbrauchbar. Sie müssen eine Karte ablegen, indem Sie mit der rechten Maustaste darauf klicken oder lange darauf drücken',

  // %ss = 'brick', %sp = 'bricks'
  'Your %sp': 'Ihre %sp', // Your bricks
  "Opponent's %sp": '%sp des Gegners', // Opponent's bricks

  '%s1. Reach %s2 to win': '%s1. Erreichen Sie %s2, um zu gewinnen', // Your tower/bricks/gems/recruits = n. Reach 100 to win
  ...
  'Review cards': 'Karten überprüfen',
  'Hide cards': 'Karten ausblenden',
  ...
  'Minimum is starting %s1 + 1 = %s0': 'Minimum ist Start-%s1 + 1 = %s0',
  'Minimum is MAX(%s1+%s2, %s3+%s4, %s5+%s6) + 1 = %s0':
    'Minimum ist MAX(%s1+%s2, %s3+%s4, %s5+%s6) + 1 = %s0',
